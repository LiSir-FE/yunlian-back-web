...
export default {
    ...
    methods: {
        ...
        detailsCourse() {
            let that = this;
            loginService.detailsCourse(that.id, {}).then(res => {
                if(res.data.success) {
                    that.pageInfo.title = res.data.datas.course.title
                    that.pageInfo.uploadVideo = res.data.datas.course.video
                    that.pageInfo.time = res.data.datas.course.time
                    that.$refs.dur.name = res.data.datas.course.time
                    that.fileList.push({
                        name: res.data.datas.course.video,
                        url: that.picHead + res.data.datas.course.video
                    });
                    if(that.fileList) {
                        document.querySelector('.uploadVideo .el-upload--text').style = 'display: none'
                    }
                    that.pageInfo.uploadContract = res.data.datas.course.file
                    that.fileLists.push({
                        name: res.data.datas.course.file,
                        url: that.picHead + res.data.datas.course.file
                    });
                    if(that.fileLists) {
                        document.querySelector('.uploadContract .el-upload--text').style = 'display: none'
                    }
                    that.pageInfo.lecturerName = res.data.datas.course.teacherId
                    that.pageInfo.courseIntroduction = res.data.datas.course.content
                    if(res.data.datas.courseWorks.length <= 0) {
                        res.data.datas.courseWorks.push({
                            cwTitle: ''
                        })
                    }
                    that.pageInfo.homeworkAfter = res.data.datas.courseWorks
                }
            }).catch(err => {
                console.log(err);
            })
        ...
        submitForm(formName) {
            ...
            that.$refs[formName].validate((valid) => {
                if (valid) {
                    loginService.editCourse(that.id, {
