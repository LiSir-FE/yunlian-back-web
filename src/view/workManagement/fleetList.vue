<template>
    <div class="wetuc-page-content">
        ...
        <el-form ref="pageInfo" :model="pageInfo" label-width="80px" :inline="true">
            ...
            <el-button type="primary" @click.prevent="addFleet">新增车队</el-button>
        ...
        <el-table :data="tableData" style="width: 100%" v-loading="tableLoading" element-loading-text="拼命加载中">
            ...
            <el-table-column label="操作" min-width="120" fixed="right" align="right">
                <template slot-scope="scope">
                    <el-button type="text" size="small" icon="el-icon-edit" @click="editClick(scope.row)">
                        修改
                    </el-button>
                    <el-button type="text" size="small" icon="el-icon-view" @click="detailsClick(scope.row)">
                        详情
                    </el-button>
                    <el-button type="text" size="small" icon="el-icon-delete" class="red" @click="deleteClick(scope.row)">
                        删除
                    </el-button>
                </template>
            </el-table-column>
    ...
    methods: {
        ...
        getfleetList() {
            ...
            loginService.getfleetList({
                ...
                startName: that.pageInfo.startCity ? that.pageInfo.startCity[0].split('/')[1] : '',
                endName: that.pageInfo.endCity ? that.pageInfo.endCity[1].split('/')[1] : '',
        ...
        addFleet() {
            this.$router.push({path: '/addFleet'});
        },
        // 修改
        editClick(row) {
        ...
        // 详情
        detailsClick(row) {
            this.$store.commit('setAddCompanyUrl', '/fleetList');
            this.$store.commit('setAddCompanyName', '车队列表');
        ...
        deleteClick(row) {
            ...
            that.$confirm('确定删除吗?', '提示', {
                ...
                loginService.deletefleetList(row.id, {}).then(res => {
                    if (res.data.success) {
                        that.$message({
                            message: '删除成功！',
                            type: 'success'
