    ...
    <div class="wetuc-page-content">
        <div class="breadcrumb-con">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>修改日历项</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
...
export default {
    ...
    methods: {
        ...
        getCalendarEditList() {
            let that = this;
            loginService.getCalendarEditList(that.id).then(res => {
                if(res.data.success) {
                    that.$nextTick(() => {
                        that.pageInfo.title = res.data.datas.title;
                        that.pageInfo.type = res.data.datas.type;
                        that.pageInfo.dateValue = res.data.datas.time;
                        that.pageInfo.hostName = res.data.datas.admins;
                        that.admins = res.data.datas.admins;
                        that.pageInfo.sendWxTemplateMsg = res.data.datas.sendWxTemplateMsg ? 1 : 2, // res.data.datas.sendWxTemplateMsg ? 1 : 2
                        that.pageInfo.jurisdiction = res.data.datas.open ? 1 : 2;
                        // 详情赋值
                        that.pageInfo.content = res.data.datas.content
                    })
