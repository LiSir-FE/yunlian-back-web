    ...
    export default {
        ...
        methods: {
            ...
            detailsGuanggao() {
                ...
                loginService.detailsGuanggao(that.id, {}).then(res => {
                    if(res.data.code == 200) {
                        let result = res.data.datas;
                        that.pageInfo.name = result.name;
                        that.pageInfo.imageUrl = process.env.IMG_URL + result.pic;
                        that.pageInfo.landingPage = result.link;
                        that.pageInfo.contacts = result.chargeName;
                        that.pageInfo.contactInformation = result.chargePhone;
                        // that.pageInfo.time = [result.onlineTime, result.offlineTime]
                        that.$set(that.pageInfo, "time", [
                            result.onlineTime,
                            result.offlineTime
                        ]);
                        setTimeout(function () {
            ...
            submitForm(formName) {
                ...
                let onlineTime = that.pageInfo.time[0] ? that.pageInfo.time[0] : '';
                let offlineTime = that.pageInfo.time[1] ? that.pageInfo.time[1] : '';
                that.$refs[formName].validate((valid) => {
                    if (valid) {
                        loginService.editGuanggaoInfo({
